...
- **环境设置工具**: 自动检查和安装依赖
- **测试图像生成**: 快速生成标准测试图像
- **形变数据采集脚本**：`collect_deformation_samples.py` 落盘撞击帧和标签
- **形变模型训练脚本**：`train_deformation_model.py` 训练轻量U-Net 生成撞击热力图
...
- **幕布形变识别模型**：计划采集撞击帧序列训练轻量神经网络生成撞击热力图，推理坐标后继续复用`get_score`完成判分

### 幕布形变识别规划
- **数据采集**：依托`impact_detector`保存撞击前后帧与差分结果，并结合`config.ini`中的已知坐标半自动标注真实落点
- **模型设计**：在PC端使用PyTorch训练轻量编码器-解码器或关键点回归模型，将幕布形变映射为像素级热力图或坐标回归输出
- **部署集成**：将模型转换为RKNN/TFLite在RK3588侧运行，替换`process_frame`中的轮廓判定逻辑，仅保留`get_score`负责区域映射
- **风险与改进**：需控制推理延迟<30ms，并通过联合传统规则实现保底逻辑，防止复杂光照导致的误判
## 版本信息
